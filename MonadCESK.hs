...
class (Address a, Monad m) => StoreVarsCont m a where
  tick      :: State a -> m ()
...
mstep :: (StoreVarsCont m a) => State a -> m (State a)
...
instance StoreVarsCont CESKx Addr where
...
instance StoreVarsCont KCFA Addr where
